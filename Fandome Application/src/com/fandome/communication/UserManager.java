...
import com.fandome.models.UserModel;
...
public class UserManager {
				...
				if (user == null 
					...
					while(!isSuccess && attemptCounter < numberOfLoginAttempt){ 
						try {
							...
							Log.d("User login", 
									String.format("error while try to signupOrLogin attempt number %s error: %s", attemptCounter, e.getMessage()));
							attemptCounter++;
						}
					}
					
					if(!isSuccess)
						return new Result<ParseUser>(new LoginException("can't login to server"));
				}
				else{
	...
	public ParseUser signupOrLogin() throws ParseException {
		...
		parseUser.setUsername(deviceId);
		String password = ApplicationManager.getAppContext().getString(R.string.usersPassword);
		parseUser.setPassword(password);
		try {
			Log.d("User login", "try to login");
			parseUser = ParseUser.logIn(deviceId, password);
	...
	public String getNickname() {
		if(nickname == null)
			nickname = DiskCache.getInstance().get("nickname");
		return nickname;
	}
	...
	public void setNewNickname(String nickname) {
		DiskCache.getInstance().add("nickname", nickname);
	}
